...
test.describe('Product List Page', () => {
  ...
  test('should expand row to show description', async ({ page }) => {
    ...
    await expect(page.locator('.p-datatable-row-expansion')).toContainText(
      'Description 1'
    );
    await expect(viewButton.locator('.pi-eye-slash')).toBeVisible();
  });

  test('should collapse row when view button is clicked again', async ({
    page,
  }) => {
    await page.goto(BASE_URL);
  ...
  test('should navigate to detailed page when row is clicked', async ({
    page,
  }) => {
    await page.goto(BASE_URL);
